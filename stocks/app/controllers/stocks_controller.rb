class StocksController < ApplicationController
  ...
  def create
    @stock = Stock.create_form_yahoo([symbol]).first
    if @stock.valid?
      render json: @stock, status: :created
    else
      render json: @stock.errors, status: :errors
    end
  end

  def update
    if @stock.update resource_params
      render json: @stock
    else
      render json: @stock.errors, status: :errors
    end
  ...
  def resource_params
    params.permit(:description, :notes, :tags, :category, :favorite, :blacklist, :ranking, :rava_symbol, :google_symbol, :investing_symbol)
  end
